...
const (
	...
	//KN_PLUGIN_FOLDER string = "/Users/jordan/.kn/plugins"
	//KN_PLUGIN_PATH   string = "/Users/jordan/Work/workspace/knative/kn-tkn-plugins/plugins"
	KN_PLUGIN_FOLDER string = "/Users/jordanzt/.kn/plugins"
	KN_PLUGIN_PATH   string = "/Users/jordanzt/Works/workspaces/kn-tkn-plugins/plugins"
	TKN_CLI_FOLDER   string = "tkn"

	PLUGIN_CLUSTERTASK_NAME string = "clustertask"
	PLUGIN_PIPELINE_NAME    string = "pipeline"
	PLUGIN_PIPELINERUN_NAME string = "pipelinerun"
	PLUGIN_RESOURCE_NAME    string = "pipelineresource"
	PLUGIN_TASK_NAME        string = "task"
	PLUGIN_TASKRUN_NAME     string = "taskrun"
)
...
func (test *e2eTest) Setup(t *testing.T, pluginname string) {
	logger := Logger{}
	...
	test.plugin = plugin{t, Logger{}}
	_, err := test.plugin.CreatePluginFolder()
	if err != nil {
		logger.Fatalf("Could not create kn plugin folder, exiting")
	}

	//_, err = test.plugin.CleanTknCli()
	//if err != nil {
	//	logger.Fatalf("Could not clean tkn folder, exiting")
	//}

	_, err = test.plugin.CopyPlugin(pluginname, []string{}, runOpts{})

	if err != nil {
		logger.Fatalf("Could not copy tkn plugin %s, exiting", pluginname)
	}
...
// Teardown clean up
func (test *e2eTest) Teardown(t *testing.T, pluginname string) {
	logger := Logger{}
	test.plugin = plugin{t, Logger{}}
	//_, err := test.plugin.CleanTknCli()
	//if err != nil {
	...
	_, err := test.plugin.DeletePlugin(pluginname, []string{}, runOpts{})

	if err != nil {
		logger.Fatalf("Could not delete tkn plugin %s, exiting", pluginname)
	}
...
func createNamespace(t *testing.T, namespace string, maxRetries int, logger Logger) (string, error) {
	...
	for retries < maxRetries {
		...
		logger.Debugf("The error is: %s", err)
		time.Sleep(RetrySleepDuration)
	}
